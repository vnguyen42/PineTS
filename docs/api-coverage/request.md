...
- **Chart-type routing (extended tickers)** — the symbol argument may carry a chart-type modifier (`"BTCUSDT;heikinashi"`, built by `ticker.heikinashi()` or inherited from `syminfo.tickerid` on a Heikin Ashi chart; see [Ticker](ticker.html)). The engine owns the Heikin Ashi transform: the secondary context fetches STANDARD candles from the provider (the modifier is stripped before any provider call) and materializes the derived Heikin Ashi bars itself. An empty-string symbol (`""`) resolves to the chart's own ticker, **modifier included**.
